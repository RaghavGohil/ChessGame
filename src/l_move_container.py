...
class MoveContainer:
    def __init__(self,position:list,size:list):
        ...
        self.move_font = pygame.font.SysFont(l_font.font_name_path,self.individual_move_text_size)
    ...
    def display(self,screen:pygame.Surface):
        ...
        l_font.render_text(screen,self.header_font_size,self.header_x,self.header_y,self.header_text,l_colors.move_container_display_text_color)
        xcount = 0 # x block count
        ycount = 0
        if (len(self.moves)-1) < self.text_fill_limit:
            for move in range(len(self.moves)):
                ...
                if (move+1) != len(self.moves): # make text darker
                    l_font.render_text_load_already(screen,self.move_font,int(self.start_offset[0]+xcount*self.individual_text_block_size[0]),int(self.start_offset[1]+ycount*self.individual_text_block_size[1]),self.moves[move],l_colors.move_container_display_text_color_dimmed)
                else: # make text lighter
                    l_font.render_text_load_already(screen,self.move_font,int(self.start_offset[0]+xcount*self.individual_text_block_size[0]),int(self.start_offset[1]+ycount*self.individual_text_block_size[1]),self.moves[move],l_colors.move_container_display_text_color)
                xcount += 1
